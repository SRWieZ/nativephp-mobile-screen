...
## Example: Barcode Display Component
...
### iOS
- Uses `UIApplication.shared.isIdleTimerDisabled` for wake lock
- Uses `UIScreen.main.brightness` for brightness control
- Brightness resets automatically when device locks

### Android
- Uses `FLAG_KEEP_SCREEN_ON` for wake lock
- Uses `WindowManager.LayoutParams.screenBrightness` for brightness
- Window-level brightness (no system permissions required)
